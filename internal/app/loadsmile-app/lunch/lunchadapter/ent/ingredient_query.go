...
func (iq *IngredientQuery) QueryRecipes() *RecipeQuery {
	query := &RecipeQuery{config: iq.config}
	step := sqlgraph.NewStep(
		sqlgraph.From(ingredient.Table, ingredient.FieldID, iq.sqlQuery()),
...
// First returns the first Ingredient entity in the query. Returns *NotFoundError when no ingredient was found.
...
//  WithRecipes tells the query-builder to eager-loads the nodes that are connected to
// the "recipes" edge. The optional arguments used to configure the query builder of the edge.
func (iq *IngredientQuery) WithRecipes(opts ...func(*RecipeQuery)) *IngredientQuery {
	query := &RecipeQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withRecipes = query
	return iq
}
	...
	if query := iq.withRecipes; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(ingredient.RecipesPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, iq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "recipes": %v`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "recipes" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Recipes = append(nodes[i].Edges.Recipes, n)
			}
		}
	}
...
func (iq *IngredientQuery) sqlQuery() *sql.Selector {
	...
	selector := builder.Select(t1.Columns(ingredient.Columns...)...).From(t1)
	if iq.sql != nil {
		selector = iq.sql
